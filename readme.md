...
**Disclaimer:** The `build-panel.sh` script will check for updates to the panel source before re-cloning. This means you can now edit the panel code in the `panel-source` directory on your own branch and then run the `build-panel.sh` script to rebuild the panel without losing your changes.
