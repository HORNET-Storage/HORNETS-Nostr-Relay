...
func NewSubscriptionManager(
	...
	for i, tier := range tiers {
		if tier.DataLimit == "" {
			log.Printf("Warning: skipping tier %d with empty DataLimit: '%s'", i, tier.DataLimit)
			continue
		}
...
func (m *SubscriptionManager) InitializeSubscriber(npub string) error {
	...
	tierLimit := m.findAppropriateTierForUser(npub, "", &allowedUsersSettings)
...
func (m *SubscriptionManager) ProcessPayment(
	...
	if paidTierBytes <= 0 {
		return fmt.Errorf("invalid paid tier storage limit: %s", tier.DataLimit)
	}
	log.Printf("Processing payment for tier: %s (%d bytes)", tier.DataLimit, paidTierBytes)
...
func (m *SubscriptionManager) processSingleSubscriptionEvent(event *nostr.Event, allowedUsersSettings *lib.AllowedUsersSettings) error {
	// Extract pubkey
	...
	newTierDataLimit := m.findAppropriateTierForUser(pubkey, activeTier, allowedUsersSettings)
	...
	newTierBytes := m.calculateStorageLimit(newTierDataLimit)
	oldTierBytes := storageInfo.TotalBytes

	// Only update if tier has actually changed
	if newTierDataLimit != activeTier || newTierBytes != oldTierBytes {
		log.Printf("Updating tier for %s from %s (%d bytes) to %s (%d bytes)", 
			pubkey, activeTier, oldTierBytes, newTierDataLimit, newTierBytes)
...
func (m *SubscriptionManager) RefreshExpiredFreeTierSubscriptions() error {
	...
	for {
		...
		for _, event := range events {
			...
			// Check if it's a free tier subscription (price = "0")
			activeTier := getTagValue(event.Tags, "active_subscription")
			isFreeTier := false
			for _, tier := range allowedUsersSettings.Tiers {
				if tier.DataLimit == activeTier && tier.Price == "0" {
					isFreeTier = true
					break
				}
			}
...
func (m *SubscriptionManager) updatePaidSubscriberRecord(
	...
	// Skip free tier subscriptions (price = "0")
	for _, allowedTier := range allowedUsersSettings.Tiers {
		if allowedTier.DataLimit == tier && allowedTier.Price == "0" {
...
func (m *SubscriptionManager) findAppropriateTierForUser(pubkey, currentTier string, allowedUsersSettings *lib.AllowedUsersSettings) string {
	switch allowedUsersSettings.Mode {
	case "free":
		// In free mode, assign users to the first available free tier
		for _, tier := range allowedUsersSettings.Tiers {
			if tier.Price == "0" {
				return tier.DataLimit
			}
		}
	...
	case "exclusive":
		...
		if m.isUserInAllowedLists(pubkey) {
			...
			for _, tier := range allowedUsersSettings.Tiers {
				return tier.DataLimit // Use first available tier
			}
		}
