...
export const Balance: React.FC = () => {
  ...
  const onFinish = (values: TopUpData) => {
    setLoading(true);
    setTimeout(() => {
      // Assuming the backend updates the balance, refetch or update state
      setLoading(false);
      setModalOpen(false);
    }, 1000);
  };
      ...
      <BaseCol span={24}>
        <NFTCard isSider>
          <BaseRow gutter={[30, 30]}>
            ...
            {/* <BaseCol span={24}>
              <S.TitleText level={4}>{t('nft.latestTransactions')}</S.TitleText>
              {transactions.map((transaction) => (
                <S.Text key={transaction.id}>
                  {moment(transaction.date).format('YYYY-MM-DD HH:mm:ss')}: {transaction.value} satoshis ({transaction.address})
                </S.Text>
              ))}
            </BaseCol> */}
          </BaseRow>
